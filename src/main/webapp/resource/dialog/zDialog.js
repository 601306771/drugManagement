...
(function() {
	...
	Dialog = function() { /****以下属性以大写开始，可以在调用show()方法前设置值****/
		this.ID = null;
		this.Width = null;
		this.Height = null;
		this.URL = null;
	...
	Dialog.IMAGESPATH = jsPath + 'images/';
	...
	Dialog.prototype.displacePath = function() {
		if (this.URL.substr(0, 7) == "http://" || this.URL.substr(0, 1) == "/" || this.URL.substr(0, 10) == "javascript") {
			return this.URL;
		} else {
	...
	Dialog.prototype.create = function() {
		...
		if ((isIE && topDoc.compatMode == "BackCompat") || ielt7) { //ie且兼容模式下或ie7以下
	...
	Dialog.prototype.show = function() {
		...
		if (Dialog._dialogArray.length > 0) {
			...
			if (HideScrollbar) {
				...
				if (!isIE) { //在firefox下改变overflow属性会导致scrollTop=0
	...
	Dialog.prototype.close = function() {
		...
		this.openerWindow.focus(); /*****释放引用，以便浏览器回收内存**/
		if (isIE && !isIE8) {
	...
	Dialog.onKeyDown = function(evt) {
		evt = window.event || evt;
		if ((evt.shiftKey && evt.keyCode == 9) || evt.keyCode == 8) { //shift键及tab键,或backspace键
			if (Dialog._dialogArray.length > 0) {
	...
	document.write('<link rel="stylesheet" type="text/css" href="' + jsPath + 'zDialog.css" />');
	if (!topWin.Dialog) {
		alert('在iframe内使用zDialog时，请确保顶层窗口也有引用zDialog.js');
	}
})();